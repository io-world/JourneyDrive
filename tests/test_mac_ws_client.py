...
from journeydrive_mac_thinclient.config import Config


@pytest.fixture
def config() -> Config:
    return Config(broker_host="192.168.1.10", machine_id="macbook-pro", api_key="a" * 32)
...
def test_handle_mouse_move(config: Config) -> None:
    with patch("journeydrive_mac_thinclient.ws_client.input_control.move_mouse", return_value=(10, 20)) as mock_move:
...
def test_handle_mouse_click(config: Config) -> None:
    with patch("journeydrive_mac_thinclient.ws_client.input_control.click_mouse") as mock_click:
...
def test_handle_keyboard_type(config: Config) -> None:
    with patch("journeydrive_mac_thinclient.ws_client.input_control.type_text", return_value=5):
...
def test_handle_clipboard_get(config: Config) -> None:
    with patch("journeydrive_mac_thinclient.ws_client.input_control.get_clipboard", return_value="hi"):
        result = ws_client._handle_clipboard_get(config, {})
    assert result == {"text": "hi"}


def test_handle_clipboard_set(config: Config) -> None:
    with patch("journeydrive_mac_thinclient.ws_client.input_control.set_clipboard") as mock_set:
...
def test_handle_keyboard_key_unknown_key_raises_dispatch_error(config: Config) -> None:
    with patch(
        "journeydrive_mac_thinclient.ws_client.input_control.send_keys",
...
async def test_dispatch_success_sends_result(config: Config) -> None:
    websocket = AsyncMock()
    with patch("journeydrive_mac_thinclient.ws_client.input_control.scroll_mouse") as mock_scroll:
...
async def test_handle_screenshot_sends_metadata_then_binary_frame(config: Config) -> None:
    websocket = AsyncMock()
    with patch(
        "journeydrive_mac_thinclient.ws_client.capture.take_screenshot",
...
async def test_run_applies_broker_pushed_config_then_stops(config: Config) -> None:
    ...
    with (
        patch("journeydrive_mac_thinclient.ws_client.websockets.connect", fake_connect),
        patch("journeydrive_mac_thinclient.ws_client.capture.list_monitors", return_value=[fake_monitor]),
    ):
...
async def test_run_raises_on_rejected_registration(config: Config) -> None:
    ...
    with patch("journeydrive_mac_thinclient.ws_client.websockets.connect", fake_connect):
